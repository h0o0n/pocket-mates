...
-- 결과 화면에서 확정된 음식점 후보 5곳 (전원 같은 목록으로 투표)
alter table public.rooms add column if not exists place_candidates jsonb;
...
create index if not exists place_votes_room_id_idx on public.place_votes (room_id);
...
grant select, insert, update on public.place_votes to anon, authenticated, service_role;
...
drop policy if exists "place_votes_update" on public.place_votes;
create policy "place_votes_select" on public.place_votes for select to anon, authenticated using (true);
create policy "place_votes_insert" on public.place_votes for insert to anon, authenticated with check (true);
create policy "place_votes_update" on public.place_votes for update to anon, authenticated using (true) with check (true);
