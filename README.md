...
1. 한 명이 **방 만들기** → 지도에서 만날 위치 선택 → 코드/링크/`?code=` 공유
2. 나머지는 **코드·링크로 입장** (같은 코드면 같은 방)
3. 혼자 또는 여러 명이 모이면 **질문 시작**
4. 각자 속도로 답변 제출
5. 먼저 끝난 사람은 대기 화면에서 다른 사람을 기다림
6. 모두 완료되면 메뉴 TOP 3 + 주변 음식점 5곳 → **투표로 오늘 갈 곳 확정**
7. 카카오/네이버 길찾기, 거리 필터 지원
